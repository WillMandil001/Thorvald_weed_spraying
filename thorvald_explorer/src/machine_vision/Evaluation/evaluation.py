...
class evaluation():
    def __init__(self):
        self.camera_FoV = 50 # in pixels
        self.wheel_centre_distance = 30
        self.wheel_width = 15

        img = cv2.imread('1_annotated.png', 0) # annotated image
        ret,self.annotated_img = cv2.threshold(img,50,255,cv2.THRESH_BINARY)

        with open('toponav.waypoints', 'rb') as wp_file:
            self.all_waypoints = pickle.load(wp_file)

    def evaluate_camera(self):
        path_img = copy.deepcopy(self.annotated_img)
        ret,path_img = cv2.threshold(path_img,255,255,cv2.THRESH_BINARY)
        previous_point = []
    ...
    def evaluate_wheels(self):
        ...
        ret,wheel_img = cv2.threshold(wheel_img,255,255,cv2.THRESH_BINARY)
        previous_point = []
        for i,row in enumerate(self.all_waypoints):
            if (i % 2) == 0:
                pass
